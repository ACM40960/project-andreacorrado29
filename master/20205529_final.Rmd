...
plot_lowess(rain$year, rain$rain, f = 1, 
            main = 'Average rain fall amount per year', ax = NULL,
...
plot_lowess(temp$year, temp$temp, f = 1, ax = NULL,
            ...
            main = 'Annual temperature over year', xaxt = 'n')
...
plot_lowess(temp$year, c(0, diff(temp$temp)), f = 1, ax = temp$year,
            xlab = 'year', ylab = 'temperature', type = 'l',
            main = 'Annual temperature increment', xaxt = 'n')
# add legend
legend('topleft', legend = c('span  100%'), lty = 1, lwd = 2,
       bty = 'n', col = c('violetred'))
...
\begin{itemize}
\item for the random intercept the assumption does not hold as we can notice a considerable discrepancy between the observed and theoretical quantiles, figure \ref{fig:ranef_qq} a). However, the intercept related variance is $\hat{\sigma}_0 =  1.84571$, which counts for the majority variance, approximately $63\%$, as we can see in table \ref{tab:re_var}. For instance, in figure \ref{fig:ranef_distr} we can observe a very wide range of variation for the random intercept estimation with very few species around the zero and all the other with $95\%$ confidence interval which does not contain the null values, hence statistically significant.
