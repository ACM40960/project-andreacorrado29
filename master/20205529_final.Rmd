...
From the model results presented in table \ref{tab:glmer_model_res}, we can observe how the coefficients related to the $year$, $rain$, and $\tau$  marginal terms $\hat{\beta}_{i,year}$, $\hat{\beta}_{rain}$, $\hat{\beta}_{\tau}$ are mainly positive. However, the model interaction terms, $\hat{\beta}_{i,year \times \omega}$ are mainly negative. Thus, we need to consider all these effects together in order to establish the relationship.
...
```
\label{fig:ranef_qq}
\caption{a) Intercept random terms qqplot; b) temperature slope andom terms qqplot; c) variance components percentage contribution}
\end{figure}

\onecolumn
\begin{figure}[bt]
```{r ranef_dotplot, message=FALSE, fig.height=6}
